...
class DoctorAppointmentsState {
  ...
  final String? error;
  final Set<int>
  processingConsultationIds; // Track consultations being processed
  ...
  DoctorAppointmentsState copyWith({
    ...
    return DoctorAppointmentsState(
      ...
      processingConsultationIds:
          processingConsultationIds ?? this.processingConsultationIds,
    );
  }
}
...
class DoctorAppointmentsCubit extends Cubit<DoctorAppointmentsState> {
  ...
  Future<void> loadAppointments(int doctorId) async {
    ...
    try {
      final allUpcoming = await _repository.getUpcomingDoctorConsultations(
        doctorId,
      );
      final past = await _repository.getPastDoctorConsultations(doctorId);
      // Separate by status and combine as needed
      final scheduled = allUpcoming.where(
        (c) => c.consultation.status == 'scheduled',
      );
      final pending = allUpcoming.where(
        (c) => c.consultation.status == 'pending',
      );
      final combinedUpcoming = <ConsultationWithDetails>[
        ...pending,
        ...scheduled,
      ];
      ...
      final upcomingFiltered = combinedUpcoming
  ...
  Future<void> uploadPrescriptionPDF(
    int consultationId,
    File pdfFile,
